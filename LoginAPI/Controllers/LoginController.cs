...
namespace SupplyDemandAPI.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<IActionResult> LoginAsync(UserLogin loginRequestDTO)
        {
            try
            {
                if (loginRequestDTO.Username == null && loginRequestDTO.Password == null && loginRequestDTO.Role == null)
                {
                ...
                // in our database.
                var user = await userRepository.AuthenticateUserAsync(loginRequestDTO.Username, loginRequestDTO.Password, loginRequestDTO.Role);
                if (user != null)
                {
                    //generate jwt token
